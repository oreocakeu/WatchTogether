...
function loadDriveStream(streamUrl, fileId) {
  // Fetch the video with auth token and create a blob URL
  // This bypasses CORS since we have an authenticated token
  addLocalMsg('system', '', 'Streaming video... this may take a moment');
  ...
  .catch(function(e) {
    // If blob approach fails (large files), fall back to token in src
    addLocalMsg('system', '', 'Trying direct stream...');
