...
function loadDriveWithToken(fileId) {
  fetch('https://www.googleapis.com/drive/v3/files/' + fileId +
        '?fields=name,mimeType,size,webContentLink&supportsAllDrives=true', {
  ...
  .then(function(meta) {
    ...
    var urls = [
      // Format 1: Direct API stream with token in header (via fetch + blob)
      { type: 'api', url: 'https://www.googleapis.com/drive/v3/files/' + fileId + '?alt=media&supportsAllDrives=true' },
      // Format 2: webContentLink from metadata
      meta.webContentLink ? { type: 'direct', url: meta.webContentLink } : null,
      // Format 3: Legacy export URL
      { type: 'direct', url: 'https://drive.google.com/uc?export=download&id=' + fileId + '&access_token=' + googleAccessToken },
    ].filter(Boolean);

    tryDriveUrls(urls, fileId);
  })
  .catch(function(e) {
    addLocalMsg('system', '', 'Could not reach Drive API: ' + e.message);
  });
}

function tryDriveUrls(urls, fileId) {
  if (!urls.length) {
    addLocalMsg('system', '', 'Could not stream this Drive file. Make sure it\'s shared as "Anyone with the link".');
    return;
  }
  ...
  if (item.type === 'api') {
    // Fetch with auth header then create blob URL for streaming
    fetch(item.url, {
      headers: { 'Authorization': 'Bearer ' + googleAccessToken },
      // Use range request to test access without downloading whole file
    })
    .then(function(r) {
    ...
    .then(function(blob) {
      ...
      startDrivePlayer(blobUrl, fileId);
    })
    .catch(function() {
      addLocalMsg('system', '', 'Trying alternative stream...');
      tryDriveUrls(rest, fileId);
    });
  } else {
    // Try direct URL in video element
    ...
    testVideo.addEventListener('loadedmetadata', function() {
      ...
      startDrivePlayer(item.url, fileId);
    });
...
function startDrivePlayer(src, fileId) {
  setupHtml5Player(src, null);
  videoType = 'html5';
