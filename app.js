...
var GOOGLE_API_KEY    = 'AIzaSyBuM6A37rrs920vAry2v6y6oPjqzLasFTM';
...
function setupDriveStream(streamUrl, fileId, fileName) {
  ...
  // Authenticated source using fetch + blob for proper streaming
  fetch(streamUrl, {
  ...
  .then(function(blob) {
    ...
    // Push video reference to Firebase so viewers know what's playing
    if (roomRef) {
      pushState({ video: currentVideo, playing: false, position: 0, playedAt: null, fileName: fileName });
    }
  })
  .catch(function(e) {
    addLocalMsg('system', '', 'Stream error: ' + e.message + '. Try a smaller file or check sharing settings.');
  });
}
